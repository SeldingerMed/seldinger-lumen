...
outcome label, calibration type, outcome success, tip-target success,
wall-perforation risk, and final-distance range. With `--check-paths`, it also
counts missing observation, mask, vessel-mask, and node-position sidecar
references without loading the arrays. Add `--json` when a CI job or notebook
needs the raw machine-readable summary.
