...
when a CI job should load referenced arrays, report mask coverage and
keypoint-to-device distances, reject empty, malformed, or shape-mismatched masks,
and catch present keypoints outside the observation frame or away from the device
mask. Add `--json` when a CI job or notebook needs the raw machine-readable summary. Use
