...
The same external-comparison harness also ran 30-episode pilot policies across Lumen, CathSim, and the SOFA/stEVE Docker stack. In these pilot checks, Lumen forward and sweep policies reached 100\% raw success on all five native Lumen navigation tasks, with safe success on all forward-policy tasks and four of five sweep-policy tasks. CathSim forward and sweep pilots reached 0\% success on the two \texttt{phantom3} branch targets. SOFA/stEVE executed BasicWireNav and ArchVariety without crashes; its best pilot rates were 20\% success on BasicWireNav and 13.3\% success on ArchVariety. These pilot results are included as executable comparator evidence and are stored with raw per-episode records.

\section{Comparison to CathSim and SOFA/BeamAdapter}

CathSim, SOFA/\allowbreak BeamAdapter, and Lumen occupy related but distinct positions in the open endovascular-simulation landscape. CathSim is the closest open autonomous-catheterization benchmark reference, with MuJoCo-based robot/device simulation, force sensing, and reinforcement-learning tasks \cite{jianu2022cathsim,jianu2024cathsim}. SOFA/\allowbreak BeamAdapter is a mature flexible-instrument simulation surface for catheters, guidewires, and coils, and has been used as the basis for SOFA mechanical-thrombectomy navigation studies \cite{allard2007sofa,beamadapter,robertshaw2024mt}. Lumen is designed around a compact public benchmark where wall safety, image labels, replayable datasets, and trained-agent scorecards are first-class outputs.
...
The practical contribution is that Lumen packages wall-safety scoring, procedural task reproducibility, multi-modal labels, trained-agent evaluation, and replay artifacts into one public benchmark release that can be inspected outside the RL loop.
