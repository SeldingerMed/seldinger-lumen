...
def main():
    ...
    print(f"{'task':18} {'tier':7} {'success':>8} {'mean_steps':>11} {'max_pen':>9}")
    for t in sc.per_task:
        steps = "-" if t["mean_steps"] is None else f"{t['mean_steps']:.1f}"
        print(f"{t['name']:18} {t['tier']:7} {t['success_rate']:>8.2f} {steps:>11} {t['max_pen']:>9.3f}")
    o = sc.overall
    print(f"\noverall: success={o['success_rate']:.2f}  worst max_pen={o['max_pen']:.3f}  "
          f"mean_return={o['mean_return']:.1f}")

    print(f"\nleaderboard ({results_dir}):")
    for rank, c in enumerate(leaderboard(results_dir), 1):
        print(f"  {rank}. {c.name:24} success={c.overall['success_rate']:.2f}  "
